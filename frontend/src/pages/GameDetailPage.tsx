...
import { excerpt, usePageMeta, useGameStructuredData } from '../seo'
...
export default function GameDetailPage() {
  ...
  useGameStructuredData({
    id: game?.id ?? 0,
    name: game?.name ?? '',
    description: excerpt(game?.description, 200) || undefined,
    image: game?.imagePath ? `/uploads/${game.imagePath}` : undefined,
  })
  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-4 sm:px-8 py-6 sm:py-10">
      ...
      <div className="mt-6 flex flex-wrap items-center gap-3">
        ...
        <Link
          to={`/timeline/${game.id}`}
          className="inline-flex items-center justify-center rounded-md border border-sky-400/60 bg-gradient-to-b from-sky-400/20 via-sky-600/10 to-transparent px-4 py-2 text-sm font-semibold tracking-[0.08em] text-sky-100 shadow-[0_0_18px_rgba(56,189,248,0.12)] transition hover:border-sky-300/80 hover:text-white"
        >
          年表 →
        </Link>
      </div>
