    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <section className="mb-10">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-semibold text-gray-200">ゲーム一覧</h2>
      ...
      <section>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-xl font-semibold text-gray-200">最新アイテム</h2>
          <Link to="/items" className="text-indigo-400 hover:underline text-sm">すべて見る →</Link>
        </div>
        {items.length === 0 ? (
          <p className="text-gray-500 text-sm">まだアイテムが登録されていません</p>
        ) : (
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {items.map((item) => (
              <Link
              ...
              >
                ...
                <div className="p-3">
                  <p className="font-medium text-gray-100 text-sm">{item.name}</p>
                  <p className="text-xs text-indigo-400">{item.gameName}</p>
                </div>
              </Link>
            ))}
          </div>
